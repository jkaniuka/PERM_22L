...
\\
\\
	Do usuniêcia sygna³u budzika planujê zastosowaæ \textbf{3 filtry pasmowozaporowe} blokuj¹ce czêstotliwoœci z zakresu o œrodku w czêstotliwoœciach odpowiednio 2kHz, 4kHz i 6kHz i o szerokoœci pasma przejœciowego 150Hz. Szerokoœæ pasma przejœciowego wybra³em zgodnie z zasad¹, ¿e im wy¿sze pasmo chcemy wyt³umiæ, tym szersze powinno byæ pasmo przejœciowe (aby zachowaæ w miarê naturalne brzmienie). Szerokoœæ pasm zaporowych wybiorê na drodze eksperymentu podczas testowania filtrów.
\end{itemize}
...
Przygotowa³em w sumie cztery filtry typu \textit{windowed-sinc} (chocia¿ wykorzystam prawdopodobnie jedynie dwa z nich):

\begin{itemize}
  \item filtr dolnoprzepustowy $\rightarrow$ \textit{lowpass(sig, fc, BW)}
...
\begin{figure}[H]
\centering
 \scalebox{0.6}{\input{./plots/HP_gain.tex}}
\caption{Filtr highpass, fc=700Hz}
\end{figure}

\begin{figure}[H]
\centering
 \scalebox{0.6}{\input{./plots/BS1_gain.tex}}
\caption{Filtr bandstop \#1, fL=5775Hz  , fH=6600Hz }
\end{figure}

\begin{figure}[H]
\centering
 \scalebox{0.6}{\input{./plots/BS2_gain.tex}}
\caption{Filtr bandstop \#2, fL=3744Hz  , fH=4538Hz }
\end{figure}

\begin{figure}[H]
\centering
 \scalebox{0.6}{\input{./plots/BS3_gain.tex}}
\caption{Filtr bandstop \#3, fL=1775Hz  , fH=2413Hz }
\end{figure}



\subsection{Analiza spektrogramu przefiltrowanego sygna³u}
Po eksperymentalnym procesie doboru szerokoœci pasm zaporowych w filtrach pasmowozaporowych uda³o siê osi¹gn¹æ znaczn¹ poprawê jakoœci nagrania. Zwiêkszy³em te¿ czêstotliwoœæ graniczn¹ filtru górnoprzepustowego do 700Hz - by³ to \textbf{kompromis pomiêdzy dostatecznym wyt³umieniem sprzê¿enia sieciowego a mo¿liwie niewielkim zniekszta³ceniem mowy}. W celu sprawdzenia, czy filtry \textit{bandstop} faktycznie blokuj¹ zadane pasmo czêstotliwoœci, wygenerowa³em spektrogram nagrania audio po edycji. Rezultat widoczny jest na poni¿szym wykresie. 
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.7]{plots/spectrogram_after}
		\caption{Spektrogram sygna³u po filtracji}
\end{figure}
...
\\
\\
	W tym miejscu mo¿na odpowiedzieæ sobie na pytanie, czy wspomniany \textit{"stukot"} nale¿y wyeliminowaæ ca³kowicie. Osobiœcie uwa¿am, ¿e nie jest to konieczne. Je¿eli przyk³adowo analityk wywiadu otrzyma³by takie nagranie to filtrowa³by je do momentu, a¿ jego treœæ (np. rozmowa) bêdzie po prostu zrozumia³a, a nie idealna i ca³kowicie pozbawiona zak³óceñ.
...
title(sprintf('Sygna³ przefiltrowany'));
\end{lstlisting}
